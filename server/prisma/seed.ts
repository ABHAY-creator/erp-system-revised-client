...
async function main() {
  ...
  const monthsAgoDate = (m: number, dayOfMonth: number = 15) => {
    return new Date(currentYear, currentMonth - m, dayOfMonth, 11, 0, 0);
  };

  const pastYearDate = (m: number, dayOfMonth: number = 15) => {
  ...
  async function createQuotationWithFlow(params: {
    ...
    orderStatus?: 'Confirmed' | 'Processing' | 'Shipment Sent' | 'Delivered';
    items: { prodIdx: number; qty: number; discount?: number }[];
  }) {
